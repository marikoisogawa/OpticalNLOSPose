...
state_net = VideoRegNet(dataset.mean.size, cfg.v_hdim, cfg.cnn_fdim, mlp_dim=cfg.mlp_dim, cnn_type=cfg.cnn_type,
                        v_net_type=cfg.v_net, v_net_param=cfg.v_net_param, cnn_rs=args.rs, causal=cfg.causal, device=device)
...
for of_np, _, _ in dataset:
    ...
    print(feature.shape, of_np.shape, take)
...
meta = {'cfg': args.cfg, 'iter': args.iter, 'rs': args.rs, 'meta': meta_id, 'time': datetime.datetime.now()}
path = os.path.expanduser('~/datasets/egopose/features/cnn_feat_%s.p' % args.out_id)
