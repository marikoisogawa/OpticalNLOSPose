...
class Config:
    ...
    def __init__(self, cfg_id, local, create_dirs=False):
        self.id = cfg_id
        cfg_name = 'config/egopose/egoforecast/%s.yml' % cfg_id
        if not os.path.exists(cfg_name):
            print("Config file doesn't exist: %s" % cfg_name)
            exit(0)
        cfg = yaml.load(open(cfg_name, 'r'))
        # create dirs
        base_dir = '/tmp/egopose' if local else '~/results/egopose'
        self.base_dir = os.path.expanduser(base_dir)
        if not local and not os.path.exists(os.path.join(self.base_dir, 'remote.txt')):
            print("Didn't mount correct directory!")
            exit(0)

        self.cfg_dir = '%s/egopose/egoforecast/%s' % (self.base_dir, cfg_id)
        ...
        self.data_dir = os.path.expanduser('~/datasets/egopose')
        ...
        self.state_net_model = '%s/egopose/statereg/%s/models/iter_%04d_inf.p' % (self.base_dir, self.state_net_cfg, self.state_net_iter)
