...
class HumanoidEnv(mujoco_env.MujocoEnv):
    ...
    def load_cnn_feature(self, cnn_feat_file):
        # self.expert_ind = 0
        # self.expert_list = expert_list
        # expert_dict = pickle.load(open(expert_feat_file, 'rb'))
        # self.expert_arr = [expert_dict[x] for x in self.expert_list]
        # self.set_expert(0)
        cnn_feat_dict, _ = pickle.load(open(cnn_feat_file, 'rb'))
        self.cnn_feat = [cnn_feat_dict[x] for x in cnn_feat_dict.keys()]
    ...
    def get_obs(self):
        ...
        elif self.cfg.obs_type == 'vgail':
            obs = self.get_vgail_obs()
        return obs
    ...
    def get_vgail_obs(self):
        data = self.data
        qpos = data.qpos.copy()
        qvel = data.qvel.copy()
        # transform velocity
        qvel[:3] = transform_vec(qvel[:3], qpos[3:7], self.cfg.obs_coord).ravel()
        root_rot = quaternion_matrix(qpos[3:7])[:3, :3]
        obs = []
        # vel
        obs.append(qvel[:6])
        # up dir
        obs.append(root_rot[:, 2])
        # pos
        ee_pos = self.get_ee_pos(self.cfg.obs_coord)
        obs.append(ee_pos)
        # head quat
        head_q = data.get_body_xquat('Head')
        obs.append(head_q)
        obs = np.concatenate(obs)
        return obs
