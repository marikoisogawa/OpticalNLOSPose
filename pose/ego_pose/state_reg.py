...
parser.add_argument('--rs', default=True)
parser.add_argument('--no-rs', dest='rs', action='store_false')
parser.add_argument('--test_bat_range', type=int, default=(20, 101))
parser.add_argument('--test_bat_step' , type=int, default=20)
args = parser.parse_args()
if args.data is None:
    args.data = args.mode if args.mode in {'train', 'test'} else 'train'

cfg = Config(args.cfg, args.local, create_dirs=(args.iter == 0))
...
state_net = VideoRegNet(state_dim, cfg.v_hdim, cfg.cnn_fdim, no_cnn=no_cnn, cnn_type=cfg.cnn_type,
                        mlp_dim=cfg.mlp_dim, v_net_type=cfg.v_net, v_net_param=cfg.v_net_param,
                        cnn_rs=(args.rs if args.mode == 'test' else True), causal=cfg.causal, device=device)
if args.iter > 0:
...
if args.mode == 'train':
    ...
    for i_epoch in range(args.iter, cfg.num_epoch):
        ...
        epoch_num_sample = 0
        epoch_loss = 0
        for of_np, traj_np, _ in dataset: # ------------------ limit range of dataset for Curriculum Learning?
    ...
    if args.test_feat is None:
        ...
        for of_np, traj_np, traj_orig_np in dataset:
            ...
            epoch_loss += loss.cpu().item() * num
            epoch_num_sample += num
            # save if dataset moved on to next take
