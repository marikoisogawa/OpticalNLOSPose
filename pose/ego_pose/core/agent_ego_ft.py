...
class AgentEgoFT(AgentPPO):
    ...
    def push_memory(self, memory, state, action, mask, next_state, reward, exp):
        v_meta = np.array([0, self.policy_vs_net.v_margin])
    ...
    def update_params(self, batch):
        ...
        cnn_feat = [self.env.fix_cnn_feat]
        self.policy_vs_net.initialize((masks, cnn_feat, v_metas)) # check self.env.cnn_feat here!
        self.value_vs_net.initialize((masks, cnn_feat, v_metas))
