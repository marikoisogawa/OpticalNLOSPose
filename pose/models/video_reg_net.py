...
from models.p2psfnet import P2PsfNet


class VideoRegNet(nn.Module):

    def __init__(self, out_dim, v_hdim, cnn_fdim, no_cnn=False, frame_shape=(3, 64, 64), mlp_dim=(300, 200),
                 cnn_type='resnet', v_net_type='lstm', v_net_param=None, cnn_rs=True, causal=False, device=None):
        super().__init__()
        ...
        if no_cnn:
            ...
            self.cnn = P2PsfNet(cnn_fdim, device=self.device, running_stats=cnn_rs, spec=spec)
    ...
    def forward(self, x):
        if self.cnn is not None:
            x = self.cnn(x.view((-1,) + self.frame_shape)).view(-1, 1, self.cnn_fdim)
        if self.v_net_type is 'no_lstm':
            x = x.view(-1, self.cnn_fdim)
        else:
            x = self.forward_v_net(x).view(-1, self.v_hdim)
        x = self.mlp(x)     # in 128, out 300 --> in 300, out 200
        x = self.linear(x)  # in 200, out 115
        return x
